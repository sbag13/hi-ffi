...
func assert_method_trait_objects() {
    print("assert_method_trait_objects")

    let testStruct = TestStruct()
    let obj = SwiftTraitImpl()
    testStruct.method_taking_trait_object(obj)
    assert(obj.trait_fn_return_int() == 12345, "Swift trait impl should still be usable")

    let rustTraitObj = testStruct.method_returning_trait_object()
    rustTraitObj.trait_simple_fn()
    assert(rustTraitObj.trait_fn_return_int() == 12345, "Method should return working trait object")
    assert(
        rustTraitObj.trait_fn_return_string() == "String from trait object",
        "Method should return trait with correct string return")
    testStruct.method_taking_trait_object(rustTraitObj)

    let staticObj = SwiftTraitImpl()
    TestStruct.static_method_taking_trait_object(staticObj)
    assert(staticObj.trait_fn_return_int() == 12345, "Static method should accept trait object")

    let staticRustTraitObj = TestStruct.static_method_returning_trait_object()
    staticRustTraitObj.trait_simple_fn()
    assert(
        staticRustTraitObj.trait_fn_return_int() == 12345,
        "Static method should return trait object")
